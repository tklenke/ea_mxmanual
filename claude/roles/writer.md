...
5. **`input/writer_todo/`** - Check for new tasks dropped by Tom (ignore any files already named with `_pending` suffix — those have been consumed). For each new file, add the task to `docs/plans/writer_todo.md`, then rename the input file with a `_pending` suffix (e.g., `task.md` → `task_pending.md`).
6. **`input/feedback/`** - Check for feedback on published wiki pages (ignore any files already named with `_pending` suffix — those have been consumed). For each new file, process it (update content as needed), then rename the input file with a `_pending` suffix (e.g., `feedback.md` → `feedback_pending.md`). If feedback has structural implications, escalate to Architect before making changes.
